...
FROM ANNOUNCEMENTS WHERE FK_ANNOUNCEMENT_USER_ID = :id ORDER BY(ID_ANNOUNCEMENT) DESC
    ...
    {
        global $pdo;
	$sql = $pdo->prepare('SELECT ANNOUNCEMENTS.ID_ANNOUNCEMENT, ANNOUNCEMENTS.TITLE, ANNOUNCEMENTS.QUANTITY, ANNOUNCEMENTS.PAY_VALUE,
(SELECT PHOTOS.URL FROM PHOTOS WHERE PHOTOS.FK_PHOTOS_ANNOUNCEMENT_ID = ANNOUNCEMENTS.ID_ANNOUNCEMENT LIMIT 1) AS PHOTO, 
(SELECT USER_REGISTER.CITY FROM USER_REGISTER WHERE USER_REGISTER.ID = ANNOUNCEMENTS.FK_ANNOUNCEMENT_USER_ID) AS CITY,
(SELECT USER_REGISTER.STATE FROM USER_REGISTER WHERE USER_REGISTER.ID = ANNOUNCEMENTS.FK_ANNOUNCEMENT_USER_ID) AS STATE,
